...
### 정리하자면...  
결국 원인은 Cross Origin일 경우, 리소스 간의 접근을 허용해줘야하는데 인증 필터로 인해 preflight request가 실패하고 있었던 것입니다.  
<br>
...
왜 dev, alpha에선 CORS filter가 없어도 정상 동작이 되고 있는가 였습니다.  
...
현재 어드민 환경은 IE이고, IE에서는 포트가 다르더라도 동일 출처로 판단되어 CORS 이슈가 발생하지 않는 것이 아닐까 추측됩니다.  
실제로 CORS 필터를 제거하고 IE에서 로컬 테스트하면 정상 동작되고 있었습니다.  
<br><br>
